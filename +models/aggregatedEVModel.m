...
function [pEV_total, SoCvec, pEV_groups, SoC_groups] = aggregatedEVModel( ...
    resNoBatt_kW, dt, numEV, pMaxEV_kW, capacityEV_kWh, prevDayMeanVec)
% aggregatedEVModel.m
% -------------------------------------------------------------------------
% Gruppen A–D | Lade-/Entlade-Strategie:
%   • Notfall-Laden bis 40 %
%   • PV-Überschuss-Laden bis 90 %
%   • Basalladen   bis 70 %, begrenzt durch prevDayMean
%   • V2G-Entladen:                         (neu 3a)
%       – Bereich   0 kW ≤ ΔP < prevDayMean
%       – nur solange SoC > 70 %
%       – max. so viel, dass Residuallast ≥ 0 kW bleibt
%   • V2G-Entladen bei hoher Last  (ΔP ≥ prevDayMean) – unverändert
% -------------------------------------------------------------------------
...
for i = 1:nSteps
    ...
    % ---------- 3) Basalladen (Laden) -----------------------------------
    elseif deltaP>=0 && deltaP<prevDayMeanVec(i)
        headroom = prevDayMeanVec(i) - deltaP;               % noch „frei” bis Grenzwert
        ...
        % (nur wenn SoC > 70 %  und solange Residuallast ≥ 0 kW bleibt)
               headroom2 = min(deltaP, prevDayMeanVec(i) - deltaP); % max. zul. Entladung
        if headroom2>0
    ...
    elseif deltaP>=prevDayMeanVec(i)
        headroom = deltaP - prevDayMeanVec(i);               % >0
